		...
		onReady: function (opts, obj) {
			...
			if (!this.overlay) {
				$.extend(this, {
					...
					margin  : D.height() > W.height() || $('form').css('overflow-y') === 'scroll' ? $('form').css('margin-right') : false,
			...
			if (obj.fixed && !isTouch) {
				if (obj.locked) {
					...
					if (this.margin !== false) {
						$('form').css('margin-right', getScalar( this.margin ) + obj.scrollbarWidth);
					}
				}
		...
		afterClose: function (opts) {
			...
			if (that.overlay && !F.isActive) {
				that.overlay.fadeOut(speed || 0, function () {
					$('form').css('margin-right', that.margin);
